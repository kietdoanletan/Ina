...
module.exports = async (client, player, track) => {
    const embed = new EmbedBuilder()
        .addFields([
            ...
            { name: 'Next in queue', value: player.queue.length > 0 ? `${escapeMarkdown(player.queue[0].title)} - ${escapeMarkdown(player.queue[0].author)} (${player.queue[0].requester.toString()})` : '*No tracks in queue*', inline: false } // Without duration for next in queue
